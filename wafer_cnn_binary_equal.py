...
count = 0
for i in range(len(labels)):
    if labels[i] == 'none' and count < 877:
        count += 1
...
onehotencoder = OneHotEncoder(
    drop='first')  # 9 categories, but we only need 8 since one will be all 0s - drop the first column
...
history = cnn.fit(datagen.flow(X_train, y_train, batch_size=32), epochs=25,
                  validation_data=(X_test, y_test))  # Fit the CNN to the augmented training data
cnn.save('wafer_cnn_binary_equal_64_datagen.h5')  # Save the model
