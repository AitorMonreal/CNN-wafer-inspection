...
# Compiling the CNN
opt = tf.keras.optimizers.Adam(learning_rate=0.0001)  # Modify the learning rate of the Adam optimizer used
'''
...
history = cnn.fit(datagen.flow(X_train, y_train, batch_size=32), epochs=25, validation_data=(X_test, y_test))  # Fit the CNN to the augmented training data
cnn.save('wafer_cnn_128_datagen.h5')  # Save the model
